    ...
    def subscribe_all(self):
        response = requests.get('https://api.zulip.com/v1/streams',
                                auth = requests.auth.HTTPBasicAuth(
                                    os.environ['ZULIP_WEATHER_EMAIL'],
                                    os.environ['ZULIP_WEATHER_KEY']))
...
class Weather(object):
    def __init__(self):
        ...
        self.lat, self.lng = 40.72078, -74.001119 # Defaults to 455 Broadway
        self.time = None
    ...
    def get_current(self):
        return self.format_forecast(self.client.currently())
    ...
    def get_tomorrow(self):
        self.set_tomorrow()
        client = forecastio.load_forecast(self.api_key,
                                          self.lat,
                                          self.lng,
                                          time = self.time)
        return '{}\n{}'.format(client.hourly().summary,
                               self.icons[self.client.hourly().icon])

    def get_week(self):
        week = self.client.daily()
        return '{}\n{}'.format(week.summary.encode('utf-8'),
                               self.icons[week.icon])

    """
    Formats a forecast output like the following:
    <icon> <text summary>, <temperature> (feels like <apparent-temperature>)
    """
    def format_forecast(self, dataPoint):
        return '{} {}, {} (feels like {})'.format(
            self.icons[dataPoint.icon],
            dataPoint.summary,
            self.temperatureCF(dataPoint.temperature),
            self.temperatureCF(dataPoint.apparentTemperature))

    """
    Gets a temperature in Fahrenheit and return integer temperatures
    in Celcius and Fahrenheit.
    """
    def temperatureCF(self, tempF):
        tempF = int(round(tempF))
        tempC = int(round((tempF - 32) * 5 / 9))
        return '**{}ºC** / **{}ºF**'.format(tempC, tempF)
